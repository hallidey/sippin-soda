...
Valid non-secret workspace settings persist locally across restarts, while the proxy remains explicitly started, ephemeral credentials are regenerated and HTTPS inspection requires a fresh per-run opt-in. A first-connection guide explains the loopback proxy address and performs an end-to-end diagnostic through a temporary local origin without using the Internet or changing system settings.
