...
| F04 Rules | v0.1 / v0.3 advanced | Ordered, persistent Development-only response rules match exact/wildcard host, path prefix and method to replace a body-compatible final status; body actions, delays and richer conditions pending | First matching status rule survives app restart; Production remains unchanged |
...
The authenticated profile and live trust preflight gate an explicitly enabled Development-only CONNECT TLS listener. Negotiated HTTP/1 requests and bounded HTTP/2 streams each receive an independent redacted capture with the upstream authority fixed to CONNECT. Development response breakpoints support bounded status/body replacement, safe replay repeats completed bodyless HTTP GET/HEAD captures against the same target, and ordered response rules persist locally and replace status after host/path/method matching. Next, extend response rules with the already bounded replacement-body action before introducing delays or request-side effects. Do not introduce silent trust-store mutation, credential forwarding to a new host or any verification bypass. Portable session export remains a later F11 slice.
