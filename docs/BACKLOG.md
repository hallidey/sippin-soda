...
| F16 TLS/production safety | v0.1 onward | Active-operation policy, effective-host classifier, visible safety class, opaque CONNECT, consent-based local CA generation/export, bounded client trust check, Development-only host certificates, strictly verified/flushed TLS bridge, readiness gate and reproducible benchmark; app interception remains off | Classification/overlap, CA generation, host/SAN validation, pass-through/interception, trust-check lifecycle, proof invalidation and Windows baseline fixtures implemented; cross-platform/C++ comparison, listener integration, OS trust automation and route revalidation pending |

## Next implementation PR

Settings now offers a single-use, 60-second loopback workflow proving trust for one chosen client without modifying trust stores or enabling interception. Next, repeat the benchmark on Windows/macOS/Linux and define how verified readiness is associated with configured clients before integrating a Development-only CONNECT route with HTTP capture. A comparable C++ workload remains required before finalizing ADR 0001. Do not introduce silent trust-store mutation or any verification bypass. Portable session export remains a later F11 slice.
