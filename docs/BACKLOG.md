...
| F16 TLS/production safety | v0.1 onward | Active-operation policy, effective-host classifier, visible safety class, opaque CONNECT, consent-based local CA generation, Development-only host certificates, strictly verified/flushed TLS bridge, readiness gate and reproducible transport benchmark; CA is not installed and app interception remains off | Classification/overlap, CA generation, host/SAN validation, pass-through, isolated interception, trust-proof invalidation and Windows baseline implemented; cross-platform/C++ comparison, listener integration, OS trust lifecycle and route revalidation pending |

## Next implementation PR

The repository now has a reproducible release benchmark and an initial Windows baseline for pass-through versus the verified TLS bridge. The workload exposed and fixed an EOF-dependent TLS forwarding stall by adding bounded directional flushes. Next, expose a bounded local trust-check workflow and repeat the benchmark on Windows/macOS/Linux before integrating a Development-only CONNECT route with HTTP capture. A comparable C++ workload remains required before finalizing ADR 0001. Do not introduce trust-store mutation or any verification bypass. Portable session export remains a later F11 slice.
