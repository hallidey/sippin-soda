...
| --- | --- | --- | --- |
| Desktop foundation | v0.1 | Tauri/React scaffold, theme, versioned local workspace settings, first-connection guide and unsigned per-user Windows installer | UI build; CI native compilation; proxy setup survives app restart without persisting secrets or HTTPS opt-in; local end-to-end diagnostic passes; manual workflow emits an NSIS artifact |
...
The authenticated profile and live trust preflight gate an explicitly enabled Development-only CONNECT TLS listener. Negotiated HTTP/1 requests and bounded HTTP/2 streams each receive an independent redacted capture with the upstream authority fixed to CONNECT. Development response breakpoints and persistent ordered rules share bounded status/body replacement, while safe replay repeats completed bodyless HTTP GET/HEAD captures against the same target. Versioned workspace settings preserve non-secret proxy configuration across restarts, Windows has an unsigned per-user NSIS packaging path, and first-run guidance validates the local forwarding path. The next adoption milestone is an installed-app smoke test and clearer runtime diagnostics; advanced delay rules can follow without blocking real use. Do not introduce silent trust-store mutation, credential forwarding to a new host or any verification bypass. Portable session export remains a later F11 slice.
