...
| F16 TLS/production safety | v0.1 onward | Active-operation policy, effective-host classifier, visible safety class, opaque CONNECT, consent-based local CA generation in OS credential storage, Development-only host certificate issuance and an isolated strictly verified TLS bridge; CA is not installed and app interception remains off | Classification/overlap, CA generation, host/SAN validation, pass-through and isolated interception fixtures implemented; listener integration, trust lifecycle and route revalidation pending |

## Next implementation PR

The engine now has an isolated TLS bridge that terminates a Development leaf and strictly verifies upstream chain and hostname before forwarding decrypted bytes. It remains disconnected from the listener, so HTTPS inspection stays explicitly disabled in the app. Next, benchmark pass-through against this bridge, define the explicit opt-in/trust readiness contract and integrate a Development-only CONNECT route with HTTP capture. Do not introduce trust-store mutation or any verification bypass. Portable session export remains a later F11 slice.
