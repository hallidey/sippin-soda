...
| F16 TLS/production safety | v0.1 onward | Active-operation policy, effective-host classifier, visible safety class, opaque CONNECT, consent-based local CA generation, Development-only host certificates, strictly verified TLS bridge and an opt-in/current-CA/client-trust readiness gate; CA is not installed and app interception remains off | Classification/overlap, CA generation, host/SAN validation, pass-through, isolated interception and trust-proof invalidation fixtures implemented; listener integration, OS trust lifecycle and route revalidation pending |

## Next implementation PR

The engine now has an isolated TLS bridge and a readiness gate requiring explicit opt-in, current CA identity and a real client trust handshake. It remains disconnected from the listener, so HTTPS inspection stays explicitly disabled in the app. Next, benchmark pass-through against the bridge and expose a bounded local trust-check workflow before integrating a Development-only CONNECT route with HTTP capture. Do not introduce trust-store mutation or any verification bypass. Portable session export remains a later F11 slice.
