...
The authenticated profile and live trust preflight now gate an explicitly enabled Development-only CONNECT TLS listener. Negotiated HTTP/1 requests and bounded HTTP/2 streams each receive an independent redacted capture with the upstream authority fixed to CONNECT. The reproducible harness now includes an HTTP/2 multiplex baseline and ADR 0001 accepts Rust; repeat measurements on Windows/macOS/Linux before setting thresholds. Next, return to user-facing v0.1 work: response breakpoints and controlled modification under the existing Development-only policy. Do not introduce silent trust-store mutation or any verification bypass. Portable session export remains a later F11 slice.
