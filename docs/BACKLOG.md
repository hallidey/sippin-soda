...
| F16 TLS/production safety | v0.1 onward | Active-operation policy, effective-host classifier, visible safety class, authenticated client routing, combined live preflight, default opaque CONNECT, consent-based local CA generation/export, bounded profile-specific client trust check, opt-in Development-only TLS listener bridge, platform upstream verification and reproducible benchmark; inner HTTPS capture remains pending | Classification/overlap, CA generation, host/SAN validation, authenticated pass-through, trust-check lifecycle, live route revalidation, TLS listener integration and Windows baseline fixtures implemented; inner HTTP capture, cross-platform/C++ comparison and OS trust automation pending |

## Next implementation PR

The authenticated profile and live trust preflight now gate an explicitly enabled Development-only CONNECT TLS bridge. The desktop uses platform certificate verification upstream, issues host-specific leaves only after route revalidation and retains opaque pass-through for Production, Unknown, non-ready and opted-out cases. Captures distinguish TLS bridge transport and count plaintext without retaining it. Next, parse the inner HTTP/1 connection and reuse the existing bounded/redacted capture pipeline for HTTPS requests and responses. Repeat the benchmark on Windows/macOS/Linux; a comparable C++ workload remains required before finalizing ADR 0001. Do not introduce silent trust-store mutation or any verification bypass. Portable session export remains a later F11 slice.
