...
- a Rustls + Hyper HTTP/2 TLS baseline with multiplexed request streams.
...
The defaults are 20 samples, four concurrent connections, a 1 MiB payload in each direction and eight HTTP/2 streams per HTTP/2 sample. Override them with `SIPPIN_BENCH_SAMPLES` (3-500), `SIPPIN_BENCH_CONCURRENCY` (1-64), `SIPPIN_BENCH_PAYLOAD_MIB` (1-64) and `SIPPIN_BENCH_H2_STREAMS` (1-64). The command emits one `SIPPIN_TLS_BENCHMARK=` JSON record suitable for archival or comparison. The HTTP/2 mode is a protocol-stack baseline, not yet a measurement of capture and redaction overhead.
...
This is a development baseline, not an acceptance threshold. Loopback results are sensitive to scheduler activity, antivirus and power state; the TLS p95 in particular needs repeated runs. Later modes may include allocations retained by earlier ones. Before finalizing performance thresholds, collect repeated Windows/macOS/Linux results and add sustained and larger-payload workloads. A C++ prototype is deferred unless Rust fails an agreed threshold.
