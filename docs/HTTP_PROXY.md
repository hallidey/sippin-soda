...
The engine readiness contract has five explicit states: Disabled, Missing CA, Expired CA, Client trust unverified and Ready. Ready requires an opt-in plus a real downstream TLS handshake using a leaf from the current CA; it is not granted by a confirmation checkbox. The resulting proof is bound to the CA fingerprint, host and leaf expiry. Opt-out clears the proof, while CA rotation/expiry or proof expiry invalidates it. The shared destination policy is evaluated after readiness, so Production and Unknown remain denied. No IPC command or UI control activates this contract yet.
