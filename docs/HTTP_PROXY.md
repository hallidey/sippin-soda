...
The inspected route advertises only ALPN `http/1.1` to the client and offers only `http/1.1` upstream. Both sides must explicitly negotiate it; absent or different upstream ALPN is rejected before application data is parsed. After both TLS handshakes and the protocol check succeed, the listener accepts persistent requests and forwards them over the same verified upstream connection. Each exchange gets a separate bounded capture and per-request timeout. A forged inner `Host` cannot redirect the verified route, and proxy credentials and hop-by-hop headers are removed. Captures expose the inner method, redacted target/query, safe headers, upstream status and optional bodies. Request JSON still uses the 1 MiB bound and stores only a redacted copy; recorded responses remain unredacted. HTTP/2 is not supported yet. Fixtures prove ALPN negotiation and rejection, two sequential exchanges on one TLS connection, authority pinning and redaction, plus rejection of untrusted or wrong-host upstream certificates.
