...
On a new workspace, Traffic shows a three-step connection guide. It starts the same explicitly controlled proxy, shows the loopback address to configure in a browser or application, and can send a diagnostic request through the proxy to a temporary loopback origin. The diagnostic never contacts the Internet, changes system proxy settings or installs certificates. A successful check proves that the local proxy forwarding path works; the client application still has to be configured with the displayed address. Finish hides the guide persistently, while Settings can reopen it.
...
The Rules workspace stores up to 64 ordered response rules in the desktop webview's local storage and sends them to the Rust engine when the proxy starts. Each enabled rule has a stable ID and name, an exact or leading-wildcard host, a path prefix, an optional HTTP method and a replacement final status. It may also replace the response with a UTF-8 body of at most 64 KiB and a validated `Content-Type`; stale entity headers and encodings are removed, length is regenerated, and optional body recording stores the replacement. The first match wins. The engine validates the complete rule set before opening the listener, invokes the shared `Modify` policy for every response and records the applied rule plus the original upstream status. Production and Unknown responses are never changed. Statuses 204, 205 and 304 are rejected because they cannot carry the configured or possibly preserved upstream body. Editing is locked while the proxy runs; stop and restart applies persisted changes. Delays and richer conditions remain future rule actions.
