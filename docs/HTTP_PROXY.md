...
An isolated engine bridge can terminate TLS from a test client with that leaf certificate and create a separate upstream TLS session. It requires an explicit root store and rustls validates both the upstream certificate chain and server name; there is no verification-bypass option. Upstream verification completes before downstream TLS, and data forwarding begins only after both handshakes succeed. Separate 16 KiB directional pumps flush each forwarded block, avoiding EOF-dependent stalls while preserving backpressure. Fixtures prove successful bidirectional plaintext transfer, rejection of an untrusted upstream certificate and rejection of a certificate for another hostname; handshake and lifetime bounds remain explicit. The bridge is not wired into the listener, so every app CONNECT remains opaque pass-through and `httpsInspection` remains false. Listener integration, HTTP capture inside TLS, trust-store install/verification/removal and rotation workflows remain subsequent slices.
...
The engine readiness contract has five explicit states: Disabled, Missing CA, Expired CA, Client trust unverified and Ready. Ready requires an opt-in plus a real downstream TLS handshake using a leaf from the current CA; it is not granted by a confirmation checkbox. The resulting proof is bound to the CA fingerprint, host and leaf expiry. Opt-out clears the proof, while CA rotation/expiry or proof expiry invalidates it. The shared destination policy is evaluated after readiness, so Production and Unknown remain denied.

Settings now exposes a client-specific trust check. Start it explicitly, copy the temporary `https://localhost:<random-port>/` URL, and open it in the same browser or runtime that will use the proxy. The endpoint listens only on IPv4 loopback, accepts one connection and expires after 60 seconds. It returns a fixed confirmation page after a trusted TLS handshake; it does not inspect or retain the request. An untrusted client receives its normal certificate error and the check reports failure. Cancel, retry, CA generation/removal and app exit discard the listener and proof. A successful check does not install the CA, prove trust for another client or enable CONNECT interception; `httpsInspection` remains false.
