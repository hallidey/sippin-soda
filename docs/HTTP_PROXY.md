...
The desktop now exposes the engine's combined preflight state. The client identity is taken from the currently running proxy's validated authentication configuration, not from a UI-supplied claim. Ready requires that identity to match a non-expired trust proof from the current non-expired CA. Proxy stopped, authentication absent, proof absent, CA changes and profile mismatch remain distinct blocked states. Ready means only that a future Development-only route has satisfied its prerequisites; the listener still performs opaque pass-through and reports HTTPS inspection inactive.
