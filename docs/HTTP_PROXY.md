...
The engine bridge can terminate TLS from a client with a host-specific leaf and create a separate upstream TLS session. There is no verification-bypass option. The desktop uses the platform certificate verifier; upstream chain and server-name validation completes before downstream TLS, and data forwarding begins only after both handshakes succeed. Separate 16 KiB directional pumps flush each forwarded block, avoiding EOF-dependent stalls while preserving backpressure. Fixtures prove successful listener integration and bidirectional plaintext transfer plus rejection of untrusted or wrong-host upstream certificates. Enabling the route requires client authentication, matching readiness and a separate per-run checkbox. Only Development destinations enter the bridge; all other routes remain opaque. `TLS bridge` captures count forwarded plaintext but do not yet expose inner methods, paths, headers, statuses or bodies.
