...
After both TLS handshakes succeed, the listener accepts one inner HTTP/1.1 request and forwards it to the fixed CONNECT authority. A forged inner `Host` cannot redirect the verified route, and proxy credentials and hop-by-hop headers are removed. The capture exposes the inner method, redacted target/query, safe headers, upstream status and optional bodies. Request JSON still uses the 1 MiB bound and stores only a redacted copy; recorded responses remain unredacted. HTTP/2 and multiple requests on one CONNECT tunnel are not supported yet. Fixtures prove this full path plus rejection of untrusted or wrong-host upstream certificates.
